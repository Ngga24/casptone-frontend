...
export default function ForgotPasswordPage({ onBack }) {
  ...
  const inputClass =
    "w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 placeholder:text-slate-400 text-sm outline-none transition-all duration-200 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10";
  const buttonClass =
    "w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm transition-all shadow-sm shadow-blue-600/10 active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex items-center justify-center px-6 font-sans">
      ...
      {toast.show && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-6 py-4 rounded-xl shadow-xl text-white transition-all transform animate-in slide-in-from-top-10 duration-300 ${
      ...
      <div className="w-full max-w-md p-8 rounded-2xl bg-white border border-slate-100 shadow-xl shadow-slate-200/40 relative">
        ...
        {step === "otp" && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            >
              {timer > 0
                ? `Kirim ulang dalam ${Math.floor(timer / 60)}:${(timer % 60).toString().padStart(2, "0")}`

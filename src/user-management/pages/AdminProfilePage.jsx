...
import Sidebar from "../components/Sidebar"; // 🔥 Pastikan path ini mengarah ke Sidebar Admin lu
import useProfile from "../hooks/useProfile";
import { User, Upload, CheckCircle2, AlertCircle, X } from "lucide-react";

export default function AdminProfilePage() {
  const navigate = useNavigate();
  const { profile, setProfile, profileImage, loading, error, updateProfile } =
    useProfile();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        "success",
        result.message || "Profil Admin berhasil diperbarui!",
      );

      // 🔥 Setelah save, balikin ke halaman utama admin
  ...
  return (
    // 🔥 Pakai layout flex bawaan Admin (sama persis kayak UserManagement / LoginLog)
        ...
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Manage your administrator personal information and profile
              picture.
            </p>
          </div>
          ...
          {toast.show && (
            <div
              className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-6 py-4 rounded-xl shadow-xl text-white transition-all transform animate-in slide-in-from-right-10 ${
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full mt-1.5 px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 outline-none transition"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className="w-full mt-1.5 px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500/20 outline-none transition bg-white"
                  >
